...
class Settings:
    ...
    def __init__(self):
        ...
        # Veles Model Configuration (Qwen2.5-32B + LoRA via vLLM on RunPod)
        self.VELES_API_URL = os.getenv("VELES_API_URL", "")
        self.VELES_API_KEY = os.getenv("VELES_API_KEY", "EMPTY")
        self.VELES_MODEL_NAME = os.getenv("VELES_MODEL_NAME", "Drushka/Veles-Finance-32B-LoRA")
        self.USE_VELES_MODEL = bool(self.VELES_API_URL)

        # Guided JSON repair via vLLM XGrammar (Phase 1.2 — constrained decoding).
        # When True: if the model produces a response that fails envelope parsing,
        # a lightweight repair call is made with guided_json constraint so the model
        # physically cannot output invalid JSON. Does NOT affect normal generation.
        self.GUIDED_JSON_REPAIR_ENABLED = os.getenv("GUIDED_JSON_REPAIR_ENABLED", "true").lower() in ("true", "1", "yes")
