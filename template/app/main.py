...
from app.core.payment import attach_payment_middleware
from app.infrastructure.episodic_store import init_episodic_store
...
async def lifespan(app: FastAPI):
    ...
    # Phase 4.2 — initialize episodic memory store (SQLite + FTS5)
    try:
        init_episodic_store()
    except Exception as e:
        logger.exception("episodic_store_init_failed", error=str(e))

    # Phase 5.1 — initialize async job store (Redis/Valkey or in-memory)
    try:
        await job_store.initialize()
    except Exception as e:
        logger.exception("job_store_init_failed", error=str(e))
...
# Must be attached before routes are included so the middleware chain is correct.
# Intercepts POST /api/v1/chatbot/chat and /chat/stream to require USDC payment.
